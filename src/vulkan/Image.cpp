...
namespace cat
{
	...
	Image::Image(Device& device, VkExtent2D extent, VkFormat format, VkImageUsageFlags usage, VkImageAspectFlags aspectFlags)
		:m_Device(device),  m_Format(format)
	{
	}

	Image::Image(Device& device, VkExtent2D extent, VkFormat format, VkImageUsageFlags usage, VkImageAspectFlags aspectFlags, VkImage image)
	{
		CreateTextureImageView();
	}
